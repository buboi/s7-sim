...
func loadConfig(path string) (Config, error) {
    ...
    for {
        ...
        case "[server]":
            ...
            addr, rack, slot := parseAddress(rec[3])
            cycle, _ := strconv.Atoi(rec[5])
            cfg.Server = ServerDef{ID: id, Address: addr, Rack: rack, Slot: slot, Cycle: time.Duration(cycle) * time.Millisecond}
        case "[point]":
            if len(rec) < 10 {
                continue
            }
...
func parseAddress(raw string) (addr string, rack int, slot int) {
    parts := strings.Split(raw, "|")
    if len(parts) == 3 {
